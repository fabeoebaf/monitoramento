...
def register_callbacks(app):
    ...
    def update_cemaden(n, est_filt):
        ...
        try:
            ...
            conn.close()
            return [], html.Div(f"Erro SQL: {e}", className="alert alert-danger m-3"), fig_empty, [], fig_empty
        ...
        if est_filt: df = df[df['nome_limpo'] == est_filt]

        ultimas = df.groupby('nome_limpo').last().reset_index()

        # --- 1. TABELA (DESTAQUE TOTAL NOS ALERTAS) ---
        ...
        style_data_conditional = [
            ...
            # Alerta Observação (Amarelo)
            {
                'if': {'filter_query': '{24h} >= 10 && {24h} < 30', 'column_id': '24h'},
                'backgroundColor': '#f1c40f', 'color': 'black', 'fontWeight': 'bold'
            },
            # Alerta Atenção (Laranja)
            {
                'if': {'filter_query': '{24h} >= 30 && {24h} < 70', 'column_id': '24h'},
                'backgroundColor': '#e67e22', 'color': 'white', 'fontWeight': 'bold'
            },
            # Alerta Crítico (Vermelho)
            {
                'if': {'filter_query': '{24h} >= 70', 'column_id': '24h'},
                'backgroundColor': '#e74c3c', 'color': 'white', 'fontWeight': 'bold'
            },
            # Normal (Verde sutil)
            {
                'if': {'filter_query': '{24h} < 10', 'column_id': '24h'},
        ...
        color_map = {
            "CRÍTICO": "#e74c3c", "ATENÇÃO": "#e67e22", 
            "OBSERVAÇÃO": "#f1c40f", "NORMAL": "#2ecc71"
        }
        ...
        # --- 3. CARDS (AGORA COM FUNDO COLORIDO SE TIVER ALERTA) ---
        cards = []
        for _, row in ultimas.iterrows():
            v24 = row['chuva_24h']
            cor = get_color_code(v24)
            
            # Define estilo do card baseado no alerta
            ...
            # Se for alerta (>= 10mm), pinta o fundo!
            if v24 >= 10:
            ...
            ], className=f"shadow-sm h-100 border-0", style=card_style), 
