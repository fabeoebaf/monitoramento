...
def get_data(source, stations, start_date, end_date, freq):
    ...
    try:
        ...
        query = f"""
        SELECT * FROM {table} 
        WHERE nome_estacao IN ({placeholders}) 
        AND data_hora BETWEEN ? AND ?
        ORDER BY data_hora ASC
        """
        
        params = stations + [f"{start_date} 00:00:00", f"{end_date} 23:59:59"]
        ...
        # Agregação (Resample)
        if freq == 'D': # Diário
            # Chuva soma, Temperatura média
            df.set_index('data_hora', inplace=True)
            
            # Agrupa por estação e faz o resample
            df_grouped = df.groupby('nome_estacao').resample('D').agg({
                'chuva_mm': 'sum',
                'temp_ar': 'mean',
                'umidade': 'mean',
                'vento_vel': 'max' # Rajada máxima do dia
            }).reset_index()
            
            # Remove dias vazios gerados pelo resample se necessário
            return df_grouped.dropna(subset=['chuva_mm', 'temp_ar'], how='all')
...
def register_callbacks(app):
    ...
    def update_report(n, source, stations, start, end, var, freq):
        ...
        coluna_tempo = 'data_hora' if 'data_hora' in df.columns else 'data_hora' # O groupby mantem o nome se reset_index
        ...
        stats['sum'] = df.groupby('nome_estacao')[var].sum().values # Adiciona soma (importante pra chuva)
